...
from algoding.research.dl_historical import (
    available_dl_models,
    build_dl_dataset,
    default_dl_models,
    run_fixed_split_dl_trace,
    run_walk_forward_dl_trace,
)
...
class DlResearchLab:
    def __init__(self, settings: Settings) -> None:
        if not settings.has_alpaca_credentials:
            raise RuntimeError("Alpaca credentials are required in .env for DL historical research.")
        self._settings = settings
        self._stock_history = AlpacaHistoricalClient(settings)
        self._yahoo_history = YahooHistoricalClient()
        ...
        for market in resolved_markets:
            ...
            for symbol, bars in series.items():
                for window_name in resolved_windows:
                    ...
                    max_sequence_length = max(model.sequence_length for model in resolved_models)
                    try:
                        dataset = build_dl_dataset(window_bars, sequence_length=max_sequence_length)
                    ...
                    for model in resolved_models:
                        try:
                            trace = run_walk_forward_dl_trace(
                                dataset=dataset,
    ...
    def report(self, limit: int = 50) -> dict[str, object]:
        self._store.ensure_schema()
        runs = self._store.list_all_strategy_runs(run_type="historical_dl_execution_aware", limit=5000)
    ...
    def run_fixed_split(
        ...
        *,
        ...
        resolved_models = self._resolve_models(models)
        max_sequence_length = max(model.sequence_length for model in resolved_models)
        buffer_bars = max_sequence_length + 40
        max_bars = train_bars + test_bars + buffer_bars
        risk_limits = self._risk_limits.with_overrides(
            stop_loss_pct=stop_loss_pct,
            trailing_stop_pct=trailing_stop_pct,
        )
        ...
        for market in resolved_markets:
            ...
            for symbol, bars in series.items():
                try:
                    dataset = build_dl_dataset(bars, sequence_length=max_sequence_length)
                except ValueError as exc:
                    skipped_runs.append(
                        {
                            "market": market,
                            "symbol": symbol,
                            "window_name": "train14y_test1y",
                            "model_name": "dataset",
                            "reason": str(exc),
                        }
                    )
                ...
                for model in resolved_models:
                    try:
                        trace = run_fixed_split_dl_trace(
                            dataset=dataset,
                            model_definition=model,
                            symbol=symbol,
                            market=market,
                            window_name=f"train14y_test1y_{timeframe}{'_rth' if regular_hours_only else ''}",
                            risk_limits=risk_limits,
                        ...
                        continue
                    replay = run_execution_aware_replay(dataset.bars[-test_bars:], trace, assumptions)
                    replay["summary"]["source_run_type"] = "historical_dl_fixed_split"
                    replay["summary"]["run_type"] = "historical_dl_fixed_split_execution_aware"
                    replay["summary"]["train_bars"] = train_bars
                    replay["summary"]["test_bars"] = test_bars
                    self._store.record_strategy_run(replay["summary"])
                    all_results.append(replay["summary"])
    ...
    def _load_market_series(
        ...
        *,
        ...
        if symbols_filter:
            allowed = {value.upper() for value in symbols_filter}
            symbols = [symbol for symbol in symbols if symbol.upper() in allowed]
        if not symbols:
            return {}
        if market == "fx":
            if timeframe.strip().lower() != "day":
                raise ValueError("FX DL research currently supports daily bars only.")
        ...
        if timeframe.strip().lower() == "day" and max_bars > 1500:
            stock_bars = self._yahoo_history.get_recent_daily_bars(symbols, max_bars)
        else:
    ...
    def _resolve_markets(markets: list[str]) -> list[str]:
        ...
        return resolved or ["commodities", "stocks"]

    @staticmethod
    def _resolve_models(models: list[str] | None) -> list[object]:
        available = {model.name: model for model in available_dl_models()}
        if not available:
            raise RuntimeError("No DL models are available. Verify that PyTorch is installed.")
        if not models:
            return default_dl_models()
        ...
        if invalid:
            raise ValueError(f"Unsupported DL models: {', '.join(invalid)}")
        return resolved
