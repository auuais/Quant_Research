...
REGISTERED_XLE_STRATEGIES: list[StrategyDefinition] = [
    StrategyDefinition(name="xle_momentum_daily", lookback=20, signal_type="momentum", threshold=0.02),
    StrategyDefinition(
        name="xle_mean_reversion_daily", lookback=5, signal_type="mean_reversion", threshold=0.03
    ),
    StrategyDefinition(name="xle_breakout_daily", lookback=15, signal_type="breakout", threshold=0.00),
    StrategyDefinition(
        name="xle_momentum_aggressive", lookback=7, signal_type="momentum", threshold=0.01
    ),
]
...
def evaluate_strategy(prices: list[float], definition: StrategyDefinition, symbol: str = "XLE") -> dict[str, object]:
    ...
    for index in range(definition.lookback, len(prices) - 1):
        ...
        signal = _latest_signal(window, definition)
...
def evaluate_registered_xle_strategies(prices: list[float]) -> list[dict[str, object]]:
    results = [evaluate_strategy(prices, definition) for definition in REGISTERED_XLE_STRATEGIES]
